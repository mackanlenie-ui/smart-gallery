...
public class MainActivity extends Activity {
    ...
    private static final String FILTER_JS="(function(){if(window.__fbAdFilterV27)return;window.__fbAdFilterV27=true;"+
      ...
      "var s=document.getElementById('adfilter-style-v27');if(!s){s=document.createElement('style');s.id='adfilter-style-v27';s.innerHTML=css;document.documentElement.appendChild(s);}"+
      ...
      "function clean(root){root=root||document;var q=root.querySelectorAll?root.querySelectorAll('span,div,a,button'):[];for(var i=0;i<q.length;i++){var e=q[i],t=norm(e.innerText||e.textContent||'');if(e.children.length<=2&&marker(t))hideCard(e)}var a=root.querySelectorAll?root.querySelectorAll('[role=article],article'):[];for(var j=0;j<a.length;j++){var l=(a[j].innerText||'').split(/\\n+/).map(function(x){return x.trim().toLowerCase()}).filter(Boolean).slice(0,14);if(l.indexOf('ad')>=0||l.indexOf('sponsored')>=0||l.indexOf('sponsrad')>=0||l.indexOf('sponsrat')>=0)a[j].setAttribute('data-adfilter-hidden','1')}}"+
    ...
    private static final String PROMO_JS="(function(){if(window.__fbWrapperPromoV27)return;window.__fbWrapperPromoV27=true;"+
      "function norm(t){return(t||'').replace(/\\s+/g,' ').trim().toLowerCase()}"+
      "function isPrompt(t){t=norm(t);return t.indexOf('skaffa facebook för android')>=0||t.indexOf('skaffa facebook for android')>=0||t.indexOf('get facebook for android')>=0||t.indexOf('facebook är bättre i appen')>=0||t.indexOf('facebook is better in the app')>=0||t==='öppna appen'||t==='öppna app'||t==='open app'||t==='skaffa appen'||t==='get app'}"+
      "function hide(e){if(!e||e===document.body||e===document.documentElement)return;var n=e.closest('[role=dialog],a,button')||e,best=n;for(var i=0;i<7&&n&&n!==document.body;i++,n=n.parentElement){var r=n.getBoundingClientRect(),txt=norm(n.innerText||n.textContent||'');if(isPrompt(txt)&&r.height>20&&r.height<Math.max(300,innerHeight*.48)&&r.width>innerWidth*.45)best=n}if(best&&best!==document.body&&best!==document.documentElement)best.style.setProperty('display','none','important')}"+
      "function clean(root){root=root||document;var q=root.querySelectorAll?root.querySelectorAll('a,button,span,div,[role=dialog]'):[];for(var i=0;i<q.length;i++){var e=q[i],t=norm(e.innerText||e.textContent||'');if(isPrompt(t))hide(e)}}"+
      "clean(document);new MutationObserver(function(ms){for(var i=0;i<ms.length;i++)for(var j=0;j<ms[i].addedNodes.length;j++){var n=ms[i].addedNodes[j];if(n&&n.nodeType===1)clean(n)}}).observe(document.documentElement,{childList:true,subtree:true});setInterval(function(){clean(document)},700)})();";

    private static final String DARK_ON_JS="(function(){var id='fbwrapper-dark-v27',s=document.getElementById(id);if(!s){s=document.createElement('style');s.id=id;s.textContent='html{background:#111!important;filter:invert(1) hue-rotate(180deg)!important}body{background:#fff!important}img,video,picture,canvas,svg image,[style*=background-image],iframe{filter:invert(1) hue-rotate(180deg)!important}';document.documentElement.appendChild(s)}})();";
    private static final String DARK_OFF_JS="(function(){var s=document.getElementById('fbwrapper-dark-v27');if(s)s.remove()})();";
    private static final String DEX_LAYOUT_JS="(function(){var id='fbwrapper-dex-v27',s=document.getElementById(id);if(!s){s=document.createElement('style');s.id=id;s.textContent='html,body{min-width:100%!important;width:100%!important}body{margin:0 auto!important}body>div:first-child,#root,[role=main]{width:100%!important;max-width:1180px!important;margin-left:auto!important;margin-right:auto!important}main,[role=main]{padding-left:24px!important;padding-right:24px!important;box-sizing:border-box!important}[role=feed]{width:min(760px,72vw)!important;max-width:760px!important;margin-left:auto!important;margin-right:auto!important}[role=article],article{max-width:760px!important;margin-left:auto!important;margin-right:auto!important}';document.documentElement.appendChild(s)}var m=document.querySelector('meta[name=viewport]');if(m)m.setAttribute('content','width=device-width, initial-scale=1.0, maximum-scale=1.0');})();";
    private static final String DEX_OFF_JS="(function(){var s=document.getElementById('fbwrapper-dex-v27');if(s)s.remove()})();";
    private static final String DEX_AD_JS="(function(){if(window.__fbDexAdFilterV27)return;window.__fbDexAdFilterV27=true;"+
      "var css='[data-fb-dex-ad-hidden=\\\"1\\\"]{display:none!important;margin:0!important;padding:0!important;height:0!important;min-height:0!important;overflow:hidden!important}';var st=document.getElementById('fb-dex-adfilter-v27');if(!st){st=document.createElement('style');st.id='fb-dex-adfilter-v27';st.textContent=css;document.documentElement.appendChild(st)}"+
      "function norm(t){return(t||'').replace(/\\s+/g,' ').trim().toLowerCase()}function compact(t){return norm(t).replace(/[^a-zåäö]/g,'')}function isMark(t){var n=norm(t),c=compact(t);return n==='ad'||n==='sponsored'||n==='sponsrad'||n==='sponsrat'||n==='sponsras'||c==='sponsored'||c==='sponsrad'||c==='sponsrat'||c==='sponsras'}"+
      "function hideFeed(e){var a=e.closest('[role=article],article');if(!a){var n=e;for(var i=0;i<8&&n&&n!==document.body;i++,n=n.parentElement){var r=n.getBoundingClientRect();if(r.width>360&&r.width<900&&r.height>100&&r.height<1800){a=n;break}}}if(a&&a!==document.body&&a!==document.documentElement)a.setAttribute('data-fb-dex-ad-hidden','1')}"+
      "function hideRail(e){var n=e,b=null;for(var i=0;i<8&&n&&n!==document.body;i++,n=n.parentElement){var r=n.getBoundingClientRect(),t=norm(n.innerText||'');if(r.width>=180&&r.width<=480&&r.height>=90&&r.height<=650&&t.length<4000){if(t.indexOf('kontakter')<0&&t.indexOf('contacts')<0&&t.indexOf('gruppchatter')<0)b=n}}if(b)b.setAttribute('data-fb-dex-ad-hidden','1');else hideFeed(e)}"+
      "function clean(root){root=root||document;var q=root.querySelectorAll?root.querySelectorAll('span,div,a,h2,h3,h4'):[];for(var i=0;i<q.length;i++){var e=q[i],t=e.innerText||e.textContent||'';if(e.children.length<=3&&isMark(t)){var r=e.getBoundingClientRect();if(r.left>innerWidth*.62)hideRail(e);else hideFeed(e)}}var arts=root.querySelectorAll?root.querySelectorAll('[role=article],article'):[];for(var j=0;j<arts.length;j++){var txt=norm(arts[j].innerText||'').split(/\\n+/).slice(0,18);for(var k=0;k<txt.length;k++){if(isMark(txt[k])){arts[j].setAttribute('data-fb-dex-ad-hidden','1');break}}}}"+
      "clean(document);new MutationObserver(function(ms){for(var i=0;i<ms.length;i++)for(var j=0;j<ms[i].addedNodes.length;j++){var n=ms[i].addedNodes[j];if(n&&n.nodeType===1)clean(n)}}).observe(document.documentElement,{childList:true,subtree:true});setInterval(function(){clean(document)},900)})();";
    ...
    private void applyDexAds(){if(web!=null&&dexMode&&getBool("adblock",true))web.evaluateJavascript(DEX_AD_JS,null);}
    private void applyPromoFilter(){if(web!=null)web.evaluateJavascript(PROMO_JS,null);}
    ...
    @Override protected void onResume(){super.onResume();if(web!=null){web.onResume();String u=web.getUrl();if(u==null||u.equals("about:blank"))web.loadUrl(homeUrl());else{applyTheme();applyPromoFilter();applyDexLayout();applyDexAds();web.evaluateJavascript(RESUME_JS,null);applyPlayback();}}}
