...
# Put CSV export beside the existing import controls on the Poster page.
needle='tools.addView(csv,clp);content.addView(tools);'
if needle in s:
    s=s.replace(needle,'tools.addView(csv,clp);content.addView(tools);TextView csvout=small("Exportera transaktioner CSV",Color.rgb(50,77,101));csvout.setOnClickListener(v->exportCsvV19());content.addView(csvout);',1)
insert=s.rfind('\n}')
