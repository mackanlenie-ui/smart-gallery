...
extra=r'''
  ...
  double categorySpentV22(String mk,String cat){double x=0;JSONArray a=arr("tx");for(int i=0;i<a.length();i++){JSONObject o=a.optJSONObject(i);if(o!=null&&"Utgift".equals(o.optString("type"))&&o.optString("date").startsWith(mk)&&cat.equals(o.optString("category")))x+=o.optDouble("amount");}return x;}
  void budgetAndPlannedAlertsV22(){if(!p.getBoolean("notifications",true))return;String mk=monthKey(Calendar.getInstance());JSONArray b=arr("catbudgets");for(int i=0;i<b.length();i++){JSONObject o=b.optJSONObject(i);if(o==null)continue;double lim=o.optDouble("amount");if(lim<=0)continue;double spent=categorySpentV22(mk,o.optString("category"));if(spent>=lim*.8)notifyNow(3100+i,"Budgeten närmar sig gränsen",o.optString("category")+" • "+fmt(spent)+" av "+fmt(lim));}}
'''
